					...
					<div class="modal-dialog">
						<div class="modal-content">
							<div class="modal-header">
								...
								<h4 class="modal-title">Add New Per Mile Cost</h4>
							</div>
                            <?php
							$attributes = array('name' => 'add_per_mile_cost');
							echo form_open('admin/add_per_mile_cost', $attributes);
							?>
							<div class="modal-body">
                                <label>Per Mile Cost<span class="symbol required"></span></label>
								<input type="number" class="form-control" name="per_mile_cost" placeholder="Per mile cost" required>
                ...
                <div class="modal fade" id="edit-panel-config" tabindex="-1" role="dialog" aria-hidden="true">
					<div class="modal-dialog">
						<div class="modal-content">
                            ...
                            <?php
							$attributes = array('name' => 'edit_per_mile_cost');
							echo form_open('admin/edit_per_mile_cost', $attributes);
							?>
							<div class="modal-body">
                                <label>Per Mile Cost<span class="symbol required"></span></label>
								<input type="number" class="form-control" name="per_mile_cost" id="permile_cost" value="" required>
                                <label>start Date<span class="symbol required"></span></label>
							...
							<div class="panel panel-default">
								<div class="panel-heading">
									...
									<div class="panel-tools">
										<a class="btn btn-xs btn-link panel-config" href="#panel-config" data-toggle="modal"> <i class="fa fa-plus"></i> Add New Cost</a>
									</div>
								</div>
								<div class="panel-body">
									<table class="table table-striped table-bordered table-hover table-full-width" id="sample_1">
										...
										<tbody>
                                         <?php
										 if(isset($per_mile_costing) && !empty($per_mile_costing))
										 {
										   $count=1;
                                           foreach($per_mile_costing as $key=>$val)
                                           {
                                        ?>
											<tr>
												<td><?php echo $count;?></td>
												<td><?php echo "$ ".$val->per_mile;if($key==0) echo "&nbsp;&nbsp;<span class='label label-danger'>Default Cost</span>";?></td>
                                                ...
                                                <td>
												  ...
												  if($key>0)
												  {
													  ...
													  {
													  ?>
														 <a href="<?php echo $this->config->base_url();?>Admin/change_status/<?php echo $val->id;?>/permileage/0" class="btn btn-bricky"><i class="fa fa-lock"></i> Inactive</a>
													  <?php  
													  }
													  else
													  {
													  ?>
														 <a href="<?php echo $this->config->base_url();?>Admin/change_status/<?php echo $val->id;?>/permileage/1" class="btn btn-success"><i class="fa fa-unlock"></i> Active</a>
													  <?php
													  }
												  }
												  ?>
												  ...
												  {
												  ?>
                                                     <a href="<?php echo $this->config->base_url();?>Admin/delete_per_mile_cost/<?php echo $val->id;?>"  onClick="return confirm('Are you sure you want to delete it?')" class="btn btn-bricky"><i class="fa fa-times"></i> Delete</a>
                                                  <?php
												  }
												  ?><?php */?>
                                                </td>
		...
		<script>
			jQuery(document).ready(function() {
			    ...
			    $('.edit_cost').click(function()
			    {
					...
					$.ajax({
							type: "POST",
							url: "<?php echo base_url('Admin/get_cost_details');?>",
							data:{id:id},
							success: function(data)
							{
								var rec = JSON.parse(data);
								$('#permile_cost').val(rec.per_mile);
